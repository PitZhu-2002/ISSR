...
class svm_undersampling_kmeans_strip3:
    def __init__(self,data,bootstrap = True,cluster = 3,random_state = 10):
        ...
        #self.ratio = math.ceil(len(self.maj_id) / len(self.minority))
    ...
    def support_vector(self, csf,random_state):
        ...
        for i in range(rate): # self.ratio可能要调整 重新计算一下不平衡比例
            ...
            svm = SVC(kernel='rbf',C=0.1,probability=True,random_state=10)#,random_state=0)#,random_state=10)#,random_state=5)#,random_state=15)#,random_state=25)#,random_state = 55)
...
if __name__ == '__main__':
    ...
    su = svm_undersampling_kmeans_strip3(data,bootstrap=False)
    su.generate()
